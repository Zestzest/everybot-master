...
client.on("message", async message => {
	...
	let rawArgs = message.content.slice(config.prefix.length); // This will be the EXACT text of the message, sans the command. See three lines below.
	let args = rawArgs.trim().split(/ +/g);
	let command = args.shift().toLowerCase();
	rawArgs = rawArgs.slice(command.length).trim();
	...
	if (message.content.startsWith(config.prefix)){
		...
		// Format: [prefix]ban [user mention] [reason]
		if (command === "ban") {
		...
		// Makes the bot visible, if it was invisible.
		...
		// Sets the bot to Do Not Disturb
		// Format: [prefix]annoy
		if (command === "annoy") {
			if (!message.member.roles.some(r => ["Admin", "Staff"].includes(r.name)))
				return message.reply("Sorry, you don't have permissions to use this!");
		...
		// Displays information about the bot commands
		// TODO: Add more information
		// Format: [prefix]help me
		if (command === "help") {
			if (!args[0]) {
		...
		// Tells who the bot's creator is
		// Format: [prefix]creator
		if (command === "creator") {
			let m = await message.channel.send("My Creator is Sean Jassenoff");
		}

		// Removes a number of messages, from 1 to 99
		// Format: [prefix]purge
		if (command === "purge") {
			let deleteCount = parseInt(args[0], 10);

			if (!deleteCount) { // TODO: consolidate this if statement.
		...
		// Gives a user a role, if possible
		// Format: [prefix]addrole [user mention] [role name]
		if (command === 'addrole') {
			console.log(args[1]);
	        if (!message.member.roles.some(r => ["Admin", "Staff"].includes(r.name)))
	            return message.reply("Sorry, you don't have permissions to use this!");
		...
		// Gives the sender the Member role
